...
FILE_NAME_EXCLUDES = ["Init","pulse"]  # 排除包含 "init" 的文件
...
def list_parquet_files(
    ...
    # 排除文件名中包含指定关键词的文件，大小写不敏感
    if file_name_excludes:
    # 确保是列表
        if not isinstance(file_name_excludes, list):
    ...
    files = [
        f for f in files
        if not any(
            exclude.lower() in os.path.basename(f).lower() 
            for exclude in file_name_excludes
        )
    ]
...
def plot_full_file(df, title, save_prefix):
    """
    显示并保存整个 parquet 文件的完整时间序列，
    不按放电片段截取。
    """
    data = df.sort_values("time_s").copy()
    ...
    axes[0].plot(
        time_h,
        data["voltage"].to_numpy(dtype=float),
        linewidth=1.0
    )
    ...
    axes[1].plot(
        time_h,
        data["current_discharge"].to_numpy(dtype=float),
        linewidth=1.0
    )
    ...
    # 完整容量曲线：优先使用原始容量列，否则按放电电流积分
    if "capacity_ah" in data.columns:
        capacity = pd.to_numeric(
            data["capacity_ah"],
            errors="coerce"
        ).to_numpy(dtype=float)
        capacity_label = "Recorded capacity / Ah"
    else:
        dt = data["time_s"].diff().fillna(0.0).to_numpy(dtype=float)
        dt = np.maximum(dt, 0.0)
        current_pos = np.maximum(
            data["current_discharge"].to_numpy(dtype=float),
            0.0
        )
        capacity = np.cumsum(current_pos * dt / 3600.0)
        capacity_label = "Accumulated discharged capacity / Ah"

    axes[2].plot(time_h, capacity, linewidth=1.0)
    axes[2].set_xlabel("Time / h")
    axes[2].set_ylabel(capacity_label)
    axes[2].grid(True)

    plt.tight_layout()

    path = save_prefix + "_full_file.png"
    fig.savefig(path, dpi=300)
    print(f"✅ 已保存完整文件图像: {path}")
...
def plot_result(result, title, save_prefix):
    ...
    # 片段仿真图只保存，不再弹窗显示；完整文件图由 plot_full_file 显示
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    for file_index, parquet_file in enumerate(parquet_files):
        ...
        try:
            ...
            # 先绘制整个 parquet 文件的完整数据，不截取片段
            full_file_base = safe_name(
                f"file{file_index}_{os.path.basename(parquet_file)}"
            )
            plot_full_file(
                df,
                title=f"Complete data: {os.path.basename(parquet_file)}",
                save_prefix=os.path.join(OUTPUT_DIR, full_file_base)
            )
