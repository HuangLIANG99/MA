...
# 第二阶段：pseudo-OCV 生成
# 优先使用 qOCV / 低倍率数据文件夹；如果 QOCV_DATA_PATH=None，则退回从 DATA_PATH 的干净低倍率片段生成。
BUILD_PSEUDO_OCV = True

# 你的 qOCV 数据文件夹 / prefix。使用时把 None 改成实际路径，例如：
# QOCV_DATA_PATH = "projects/j8005-metabatt/Metabatt/VTC/你的_qOCV_folder"
QOCV_DATA_PATH = None
QOCV_USE_S3 = USE_S3
QOCV_MAX_FILES = None
...
# qOCV 通常是 C/20、C/10 或 0.05C~0.1C；这里先放宽到 0.20C。
PSEUDO_OCV_MAX_C_RATE = 0.20
...
def list_parquet_files(
    ...
    if use_s3:
        ...
        if no_proto.lower().endswith((".parquet", ".csv")):
            ...
            pattern_parquet = no_proto.rstrip("/") + "/**/*.parquet"
            pattern_csv = no_proto.rstrip("/") + "/**/*.csv"
            files = fs.glob(pattern_parquet) + fs.glob(pattern_csv)
    ...
    if len(files) == 0:
        raise FileNotFoundError("没有找到 parquet/csv 文件，请检查 DATA_PATH/QOCV_DATA_PATH。")
...
def read_one_parquet(file_path, use_s3=True):
    """
    读取单个 parquet/csv 文件。函数名保留 read_one_parquet，是为了兼容原主程序。
    """
    print("\n" + "=" * 80)
    print(f"📄 正在读取: {file_path}")

    suffix = str(file_path).lower().split("?")[0]
...
def collect_clean_discharge_segments_from_files(
    parquet_files,
    use_s3,
    nominal_capacity_ah,
    context_label,
    ...
    """
    统一执行：读取文件 -> 预处理 -> 提取放电片段 -> 质量筛选。

    返回：
    - segment_manifest: 片段统计与质量标签；
    - segment_records: {(source_file_index, global_segment_id): segment_df}
    """
    all_manifest_rows = []
    segment_records = {}

    for source_file_index, parquet_file in enumerate(parquet_files):
        ...
        try:
            ...
            if len(discharge_segments) == 0:
                print(f"⚠️ [{context_label}] 这个文件没有找到有效放电片段，跳过。")
            ...
            discharge_summary["source_file_index"] = source_file_index
            discharge_summary["file_index"] = source_file_index
            discharge_summary["file_path"] = parquet_file
            discharge_summary["source_context"] = context_label

            print(f"\n📋 [{context_label}] 当前文件放电片段统计（前10行，已优先显示干净片段）:")
            display_cols = [
                "global_segment_id",
            ...
            for seg in discharge_segments:
                ...
                segment_records[(source_file_index, segment_id)] = seg

        except Exception as e:
            print(f"\n❌ [{context_label}] 当前文件处理失败:")
    ...
    return segment_manifest, segment_records
...
def plot_result(result, title, save_prefix):
    """
    只静默保存图片，不弹出窗口。
    片段很多时，interactive display 会导致一个窗口一个窗口手动关闭，非常慢。
    """
    ...
    ax2.axhline(0.0, linestyle="--", linewidth=1.0, alpha=0.7)
...
if __name__ == "__main__":
    ...
    # 第一遍：只做 DATA_PATH 数据读取、预处理、放电片段提取与质量筛选。
    # 这样可以先形成第一步所需的 clean segment manifest，再进入 pseudo-OCV 与前向仿真。
    segment_manifest, segment_records = collect_clean_discharge_segments_from_files(
        parquet_files=parquet_files,
        use_s3=USE_S3,
        nominal_capacity_ah=NOMINAL_CAPACITY_AH,
        context_label="main_ageing",
    )
    ...
    # 第二步：构造 NCA/VTC6 pseudo-OCV 表并加载。
    # 优先使用 QOCV_DATA_PATH 指向的 qOCV / 低倍率文件夹；
    # 如果未设置，则退回从 DATA_PATH 的干净低倍率片段构造 pseudo-OCV。
    pseudo_source_manifest = segment_manifest
    pseudo_source_records = segment_records

    if QOCV_DATA_PATH is not None:
        print("\n🔎 检测到 QOCV_DATA_PATH，将从独立 qOCV 文件夹构造 pseudo-OCV。")
        ...
        qocv_manifest, qocv_segment_records = collect_clean_discharge_segments_from_files(
            parquet_files=qocv_files,
            use_s3=QOCV_USE_S3,
            nominal_capacity_ah=NOMINAL_CAPACITY_AH,
            context_label="qocv",
        )

        qocv_manifest_path = os.path.join(
            OUTPUT_DIR,
            "qocv_discharge_segment_manifest.csv"
        )
        qocv_manifest.to_csv(qocv_manifest_path, index=False)
        print("\n✅ qOCV 片段 manifest 已保存。")
        print(f"   {qocv_manifest_path}")

        if not qocv_manifest.empty:
            pseudo_source_manifest = qocv_manifest
            pseudo_source_records = qocv_segment_records
        else:
            print("⚠️ qOCV 文件夹没有得到可用片段，将退回 main_ageing 片段生成 pseudo-OCV。")
    else:
        print("\n⚠️ QOCV_DATA_PATH=None，将从 main_ageing 数据中尝试生成 pseudo-OCV。")
        print("   你已有 qOCV 文件夹，建议把 QOCV_DATA_PATH 改成该文件夹路径。")

    pseudo_ocv_path = build_pseudo_ocv_from_clean_segments(
        segment_records=pseudo_source_records,
        manifest_df=pseudo_source_manifest,
    ...
    print("\n🎉 完成：第一步干净恒流片段筛选 + 第二步 pseudo-OCV 生成 + P2D 前向验证。")
